...
3. The second node is the `Wait for Response` node with the result name `username`. The response from the contact will be saved in this result and can be referenced as `@results.demo_username`.
